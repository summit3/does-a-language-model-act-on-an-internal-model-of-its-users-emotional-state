...
LAB = Path("data/phase2_qa_labels.csv"); prev = []
if LAB.exists():
    prev = list(csv.DictReader(open(LAB, newline="", encoding="utf-8"))); done = {l["text"] for l in prev}
    rows = [r for r in rows if r["text"] not in done]; print(f"{len(prev)} rows already labelled; classifying {len(rows)} new rows")
...
for i, r in enumerate(rows):
    ...
    out.append({"id": r["id"], "condition": r["condition"], "split": r["split"], "text": r["text"], "judge_label": m.group(0) if m else "other", "judge_raw": reply.strip()[:40]})
    if (i + 1) % 50 == 0: print(f"  {i+1}/{len(rows)} in {time.time()-t0:.0f}s", flush=True)
with open(LAB, "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=["id", "condition", "split", "text", "judge_label", "judge_raw"], lineterminator="\n"); w.writeheader(); w.writerows(prev + out)
