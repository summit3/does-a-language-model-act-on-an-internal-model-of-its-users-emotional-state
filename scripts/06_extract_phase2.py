...
and results/phase2_activation_index.csv (row order = tensor order). Layer i = output of block i.
"""
import csv, sys, time
...
Path("activations").mkdir(exist_ok=True); Path("results").mkdir(exist_ok=True)
...
with open("results/phase2_activation_index.csv", "w", newline="", encoding="utf-8") as f:
...
print(f"done: shape {tuple(acts.shape)} float16, {acts.numel()*2/1e6:.0f} MB, {dt:.0f}s for {len(rows)} rows -> activations/phase2_acts_{tag}.pt, results/phase2_activation_index.csv")
